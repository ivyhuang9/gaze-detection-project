...
def compare_data(original_tsv_path, new_tsv_path, video_start_time):
    ...
    Parameters:
        original_tsv_path (String): path to the TSV file containing the original data (i.e. the standard 
            being comparing to). Assumes that it is in the Lookit format (i.e. the format generated by 
            using Khaled's script on the Marchman data).
        new_tsv_path (String): path to the TSV file containing the new data (i.e. the data trying to find 
            accuracy of). Should be in the PsychDS format. This can be the output of the reformat_data 
            function.
